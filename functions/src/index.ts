...
export const updatePostDescription = functions.firestore.document('post/{postId}').onWrite((change, context) => {
  ...
  if (bContent !== aContent) {
    return after.ref.set({description: (aContent ?? '').slice(0, 200)}, {merge: true})
  }
  return;
})
